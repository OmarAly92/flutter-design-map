...
void main() {
  ...
  test('detects auto_route mode for the fixture', () {
    expect(detectRoutingMode(fixtureRoot), RoutingMode.autoRoute);
  });

  test('parses AutoRouterConfig routes, edges, scheme, and sheet hints', () {
    ...
    expect(details.file, 'lib/screens/details_page.dart');
    expect(
    ...
    expect(
      graph.edges.any(
        (Edge edge) => edge.from == 'HomeRoute' && edge.to == 'DetailsRoute',
      ),
      isTrue,
    );
  });
